...
class Scene(gtk.DrawingArea):
    ...
    def get_sprite_at_position(self, x, y):
        ...
        x,y = self.inverseMatrix.transform_point(x,y)
    ...
    def __check_mouse(self, x, y):
        ...
        cursor = gtk.gdk.LEFT_PTR # default
    ...
    def __on_mouse_move(self, area, event):
        ...
        scaledX,scaledY = self.inverseMatrix.transform_point(event.x,event.y)
           ...
           and gtk.gdk.BUTTON1_MASK & event.state:
            ...
            if not self.__drag_started:
                drag_started = (self.__drag_start_x is not None and \
                               (self.__drag_start_x - scaledX) ** 2 + \
                               (self.__drag_start_y - scaledY) ** 2 > self.drag_distance ** 2)
            ...
            if self.__drag_started:
                diff_x, diff_y = scaledX - self.__drag_start_x, scaledY - self.__drag_start_y
    ...
    def __on_button_press(self, area, event):
        ...
        self.__drag_start_x, self.__drag_start_y = self.inverseMatrix.transform_point(event.x, event.y)
